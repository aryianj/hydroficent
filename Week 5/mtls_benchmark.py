...
TLS_PORT = 8885       # One-way TLS broker
MTLS_PORT = 8883     # Mutual TLS broker
...
def measure_latency(port, count, label, use_mtls=False):
    ...
    sub_client = mqtt.Client(client_id=f"bench-sub-{port}")
    ...
    pub_client = mqtt.Client(client_id=f"bench-pub-{port}")
